...
SELECT
    *,
    CurrentMonthSales - PerviousMonthSales AS MOM_change,
    ROUND(CAST((CurrentMonthSales - PerviousMonthSales) AS FLOAT) 
    / PerviousMonthSales * 100 , 1)
FROM (
SELECT
...
SELECT 
    CustomerID,
    AVG(DaysUntilNextOrder) AS AvgOrder,
